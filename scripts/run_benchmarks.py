...
async def run_benchmark():
    ...
    with open("benchmarks/golden_tests.json") as f:
        tests = json.load(f)
    ...
    # Pricing for Qwen Open-Source
    # (approx $0 for local, log 0 unless API is used)
    ...
    console.print(
        f"[bold blue]🚀 Starting Phase 1 Benchmark "
        f"on {len(tests)} test cases...[/bold blue]"
    )
        ...
        for t in tests:
            ...
            try:
                res = await pipeline.query(
                    question=t["question"], collection="phase1-hardened", top_k=3
                )
    ...
    console.print("Results saved to [cyan]benchmarks/phase1_baseline.json[/cyan]")
